...
- **"But an AI agent can scrape my whole site":** distinguish three cases. (a) An operator telling an AI agent to bulk-download the catalog — that traffic arrives as generic HTTP tooling (python/curl/headless), *not* as a Claude/OpenAI bot, and rules 3+4 are built precisely for it. (b) Verified AI *user* fetchers (Claude-User, ChatGPT-User) fetching one page to answer one question with a citation — that's the `llm` channel; keep it. (c) The middle case — slow extraction *through* those user fetchers — is capped by rule **4b** (`data/cloudflare-bot-rules.md`): >20 pages/min per IP gets challenged, which citation-scale use never hits. What no firewall can prevent is AI models *knowing* your prices because they read your pages — but that's the GEO strategy working as intended: the answer carries your name and link (`09`). The defense against misuse of facts is §5 (dated, branded, attributed data), not blocking the reader.
